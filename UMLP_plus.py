...
class channel_est_pruning(nn.Module):
    def __init__(self, in_ch, out_ch, Hn,Hw, deep_supervision=False):
        ...
        self.conv0_0 = nn.Sequential(
            # conv_block1(in_ch, filters[0],3,1,1,1),
            nn.Conv2d(in_ch, filters[0], kernel_size=1, stride=1, padding=0),
            Rearrange('b c h w -> b h w c'),
            PermutatorBlock(dim1 = filters[0], dim2 = Hn*seg2, dim3 = Hw*seg2, segment_dim=seg_dim2),
            Rearrange('b h w c-> b c h w')
            )
        ...
        self.conv1_0 = nn.Sequential(
            conv_block1(filters[0], filters[1],3,2,1,1),
        ...
        self.conv2_0 = nn.Sequential(
            conv_block1(filters[1], filters[2],3,2,1,1),
        ...
        self.conv0_1 = nn.Sequential(
            # conv_block1(filters[1], filters[0],3,1,1,1),
        ...
        self.conv1_1 = nn.Sequential(
            # conv_block1(filters[2], filters[1],3,1,1,1),
        ...
        self.conv0_2 = nn.Sequential(
            # conv_block1(filters[0]*3+in_ch, filters[0],3,1,1,1),
            nn.Conv2d(filters[0]*3+in_ch, filters[0], kernel_size=1, stride=1, padding=0),
            Rearrange('b c h w -> b h w c'),
            PermutatorBlock(dim1 = filters[0], dim2 = Hn*seg2, dim3 = Hw*seg2, segment_dim=seg_dim2),
            Rearrange('b h w c-> b c h w')
            )
        ...
        self.Up1_0 = nn.Sequential(
            # up_conv(filters[1], filters[0],3,1,1),
            up_conv(filters[1], filters[0],1,1,0),
            # Rearrange('b c h w -> b h w c'),
            # PermutatorBlock(dim1 = filters[0], dim2 = Hn*seg2, dim3 = Hw*seg2, segment_dim=seg_dim2),
            # Rearrange('b h w c-> b c h w')
        )
        ...
        self.Up2_0 = nn.Sequential(
            # up_conv(filters[2], filters[1],3,1,1),
            up_conv(filters[2], filters[1],1,1,0),
            # Rearrange('b c h w -> b h w c'),
            # PermutatorBlock(dim1 = filters[1], dim2 = Hn//2*seg1, dim3 = Hw//2*seg1, segment_dim=seg_dim1),
            # Rearrange('b h w c-> b c h w')
        )
    ...
    def forward(self, x):
        ...
        x1_1 = self.conv1_1(torch.cat([x1_0, self.Up2_0(x2_0)], 1))
        # x0_2 = self.conv0_2(torch.cat([x,x0_0, self.Up2_2(x2_0), self.Up0_2(x1_1)], 1))
        x0_2 = self.conv0_2(torch.cat([x,x0_0, x0_1, self.Up0_2(x1_1)], 1))
